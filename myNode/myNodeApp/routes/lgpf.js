...
router.post('/search', async (ctx, next) => {
	...
	if (lynumber == '' && name == '') {
		...
		//查询数据库
		a = await query('SELECT * FROM lgpf ORDER BY time DESC limit ' + toppageIndex + ',' + pagesize + '');
		// 查询姓名
		lgname = await query('SELECT name FROM lgpf ORDER BY time DESC limit ' + toppageIndex + ',' + pagesize + '');
		// 查询品格分
		pgf = await query('SELECT pgf FROM lgpf ORDER BY time DESC limit ' + toppageIndex + ',' + pagesize + '');
		// 查询服务分
		fwf = await query('SELECT fwf FROM lgpf ORDER BY time DESC limit ' + toppageIndex + ',' + pagesize + '');
		// 查询责任心分
		zrx = await query('SELECT zrx FROM lgpf ORDER BY time DESC limit ' + toppageIndex + ',' + pagesize + '');
...
// 根据lynumber返回对应楼管
router.post('/getSuguan', async (ctx, next) => {
	console.log(ctx.request.body);
	lynumber=ctx.request.body.lynumber;
	// 查询楼管
	let a = await query(
		`select leader from lynumber where lynumber = '${lynumber}'`);
	ctx.body={
		code:200,
		data:a[0].leader
	}
