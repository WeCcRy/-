...
router.post('/', async (ctx, next) => {
	...
	//查询数据库
	let a = await query('SELECT * FROM lynumber where state = 1 ORDER BY lynumber limit ' + toppageIndex + ',' + pagesize + '');
	// console.log(a);
...
router.post('/lyname', async (ctx, next) => {
	console.log(ctx.request.body);
	//查询数据库
	let a = await query('SELECT lynumber FROM lynumber where state = 1 ORDER BY lynumber ');
	// console.log(a);
	// 格式化数据类型
	var dataString = JSON.stringify(a);
	var result = JSON.parse(dataString);
...
router.post('/search', async (ctx, next) => {
	...
	var a = '';
		//根据楼宇查询数据库
		a = await query(`SELECT * FROM lynumber where lynumber = '${lynumber}' && state = 1 `);
	//  if(lynumber == '' && userName == ''){
	// 	//查询全部数据库
	// 	a = await query(`SELECT * FROM lynumber where state = 1 ORDER BY lynumber`);
	// }
...
router.post('/add', async (ctx, next) => {
	...
	// 将信息添加到数据库
	let memberExists = await query(`SELECT * FROM lynumber WHERE lynumber = '${lynumber}'`);
