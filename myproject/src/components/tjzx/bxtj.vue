<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 添加损耗的对话框 -->
      <el-dialog title="添加损耗" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
        <!-- 内容主体区 -->
...
export default {
  ...
  methods: {
    ...
    oneQuest() {
      var _this = this;
      this.$http
        .post("http://localhost/bxtj/search", {
    ...
    addUser() {
      ...
      this.$refs.addFormRef.validate(valid => {
        ...
        this.$http
          .post("http://localhost/bxtj/add", this.addForm)
          .then(function(response) {
            console.log(response.data);
            if (response.data.code == 200) {
              _this.oneQuest();
