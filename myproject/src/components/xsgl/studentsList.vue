  ...
  <div>
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="学院">
              <el-select v-model="xueyuan" placeholder="请选择学院" clearable>
                <el-option
          ...
          <!-- 导出Excel -->
          <el-col :span="3">
            <el-button type="warning" icon="el-icon-download" @click="exportTemplate">获取导入模板</el-button>
          </el-col>
          <el-col :span="3">
            <!-- 导入Excel -->
            <el-upload
    ...
    <el-card>
      ...
      <el-dialog title="添加学生" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
        <!-- 内容主体区 -->
        <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="128px">
          ...
          <el-form-item label="学院" prop="xueyuan">
            <el-select v-model="addForm.xueyuan" placeholder="请选择学院" clearable>
              <el-option
                  v-for="item in xyList"
                  :key="item.id"
                  :label="item.xueyuan"
      ...
      >
        <!-- 内容主体 -->
        <el-form :model="viewForm" ref="viewFormRef" label-width="128px">
          <el-row>
            <el-col :span="6">
              <el-form-item label="学号" prop="stuNumber">
              ...
              <el-form-item label="学院" prop="xueyuan">
                <el-select v-model="viewForm.xueyuan" placeholder="请选择学院" clearable>
                  <el-option
                      v-for="item in xyList"
                      :key="item.id"
                      :label="item.xueyuan"
            ...
            <el-col :span="6">
              ...
              <el-form-item label="所在楼宇" prop="lyNumber">
                <el-select v-model="viewForm.lyNumber" placeholder="请选择楼宇号" clearable>
                  <el-option
                      v-for="item in lyNamelist"
                      :key="item.id"
                      :label="item.lynumber"
...
export default {
  ...
  methods: {
    ...
    oneQuest() {
      var _this = this;
      this.$http
        .post("http://localhost/studentsList/search", {
        ...
        try {
          ...
          for (var i = 0; i < ws.length; i++) {
            var sheetData = {
              ...
              fdytel:ws[i]['辅导员电话'],
              family:ws[i]['紧急联系人'],
              familytel:ws[i]['紧急联系人电话'],
            };
            that.outputs.push(sheetData);
          }
          console.log(that.outputs);
          this.$http
              .post("http://localhost/studentsList/batchAdd", that.outputs)
              .then(function(response) {
                console.log(response.data);
                if (response.data.code == 200) {
                  console.log("成功")
    ...
    exportTemplate(){
        require.ensure([], () => {
          ...
          export_json_to_excel(tHeader, [], "批量导入模板");
        });
    },
    // 监听添加用户对话框的关闭事件
    addDialogClosed() {
    ...
    addUser() {
      ...
      this.$refs.addFormRef.validate(valid => {
        ...
        this.$http
          .post("http://localhost/studentsList/add", this.addForm)
          .then(function(response) {
            console.log(response.data);
            if (response.data.code == 200) {
              _this.oneQuest();
    ...
    editUserInfo() {
      ...
      this.$refs.viewFormRef.validate(valid => {
        ...
        // 如果预校验成功则发起修改用户信息的请求
        this.$http
          .post("http://localhost/studentsList/edit", this.viewForm)
          .then(function(response) {
            console.log(response.data);
            if (response.data.code == 200) {
              _this.oneQuest();
    ...
    async removeStudentById(id) {
      ...
      this.$http
        .post("http://localhost/studentsList/del", {
          id: id
        })
        .then(function(response) {
          console.log(response.data);
